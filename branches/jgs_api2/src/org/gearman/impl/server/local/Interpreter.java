...
final class Interpreter {
	...
	private final void text_packet(final GearmanPacket packet, final Client client) {
		final String pkt= new String(packet.toBytes(), GearmanImplConstants.CHARSET);
	...
	private final void text_maxqueue(final String[] args, final Client client) {
		final byte[] funcName = args[1].getBytes(GearmanImplConstants.CHARSET);
		if(funcName==null) {
...
//		this.server.closePort(client.getLocalPort());
	...
	private final void set_client_id(final GearmanPacket packet, final Client client) {
		...
		final String clientId = new String(clientIdBytes,GearmanImplConstants.CHARSET);
		...
		if(job==null) {
			...
			try {
				if(job.isBackground() && this.persistence!=null) {
					persistence.remove(new ServerPersistable(job));
				}
			} catch (Exception e) {
				// TODO log issue
			}
