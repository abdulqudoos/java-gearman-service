...
abstract class JobAbstract implements Job, ClientDisconnectListener {
	...
	public final GearmanPacket createJobAssignPacket() {
		return GearmanPacket.createJOB_ASSIGN(jobHandle.getBytes(), this.getFunction().getName().toString(GearmanImplConstants.CHARSET), data);
	}
	...
	private static final byte[] initJobHandle() {
		byte[] user;
		try {
			user = java.net.InetAddress.getLocalHost().getHostName().getBytes(GearmanImplConstants.CHARSET);
	...
	private synchronized static final byte[] getNextJobHandle() {
		final byte[] jobNumber = Long.toString(jobHandleNumber).getBytes(GearmanImplConstants.CHARSET);
