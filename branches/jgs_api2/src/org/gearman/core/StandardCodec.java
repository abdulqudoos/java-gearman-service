...
//	private static final int TYPE_POS		= 4;
	...
	private final void body(final GearmanCodecChannel<Integer> channel) {
		try {
			...
			GearmanImplConstants.LOGGER.log(Level.WARNING, "unexpected exception", e);
		}
	}
	...
	private final void text(final GearmanCodecChannel<Integer> channel) {
		try {
			...
			try {
				...
				final String str = new String(strBytes, GearmanImplConstants.CHARSET);
			...
			GearmanImplConstants.LOGGER.log(Level.WARNING, "unexpected exception", th);
		}
	}
	...
	private static final byte[][] parseArguments(final byte[] body, final Type type) {
		...
		try {
			...
			GearmanImplConstants.LOGGER.log(Level.WARNING, "failed to byte buffer", e);
		}
